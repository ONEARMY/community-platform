...
if (SITE === 'emulated_site') {
  ...
  logger.debug('Connecting realTimeDatabase on port ', 4006)
  realTimeDatabase.useEmulator('localhost', 4006)

  logger.debug('Connecting storage on port ', 4007)
  storage.useEmulator('localhost', 4007)
}

export { realTimeDatabase, storage, auth, functions, firestore }
