...
try {
  ...
  console.error(`Failed to create directory for ${componentName}: ${error.message}`);
  process.exit(1);
}
...
function createComponentFileFromTemplate(
  ...
  const newFileName = templateName.replace('{componentName}', componentName).replace('.mst', '');
  const newFilePath = resolve(componentPath, newFileName);

  console.log(`Writing: ${newFilePath}`);
  writeFileSync(newFilePath, Mustache.render(fileTemplate, { ComponentName: componentName }));
}
