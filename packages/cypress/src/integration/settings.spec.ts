...
const change_settings_user = MOCK_DATA.users.change_settings
const change_settings_space_user = MOCK_DATA.users.change_settings_space
...
describe('[Settings]', () => {
  ...
  describe('[Precious Plastic]', () => {
    ...
    it('Can create member', () => {
      ...
      cy.signIn(change_settings_user.email, change_settings_user.password)
      cy.visit('/settings')
      cy.wait(5000)
      ...
      cy.step('Updated settings display on profile')
      cy.visit(`u/${change_settings_user.userName}`)
      cy.contains(tag)
    })
    ...
    it('Can create space', () => {
      ...
      cy.signIn(
        change_settings_space_user.email,
        change_settings_space_user.password,
      )
      ...
      cy.step('Updated settings display on profile')
      cy.visit(`u/${change_settings_space_user.userName}`)
      cy.contains(change_settings_space_user.userName)
      cy.contains(displayName)
      cy.contains(description)
      cy.contains(tag)
