...
interface IProps {
  errorsListSet: (IErrorsListSet | undefined)[]
  isVisible: boolean
}

export const ErrorsContainer = ({ errorsListSet, isVisible }: IProps) => {
  if (!isVisible || errorsListSet === undefined || errorsListSet.length === 0)
    return null
...
const ErrorsTitle = (props: { title?: string }) => {
  const { title } = props
