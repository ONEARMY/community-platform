...
export class UserStore {
  ...
  public async getUserProfile(userName: string) {
    const ref = await afs.doc(`usersV2${userName}`).get()
  ...
  public async updateUserProfile(values: Partial<IUser>) {
    ...
    await Database.setDoc(`usersV2${user.userName}`, update)
  ...
  private async _createUserProfile(userName: string) {
    ...
    const user: IUser = {
      ...Database.generateDocMeta('usersV2', userName),
      _authID: authUser.uid,
      userName,
      verified: false,
    }
    await Database.setDoc(`usersV2${userName}`, user)
