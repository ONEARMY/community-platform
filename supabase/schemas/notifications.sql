...
AS $function$
BEGIN
    SELECT DISTINCT u.email, p.id, p.created_at
    FROM subscribers s
    ...
    WHERE s.content_id = p_content_id
        ...
        -- Only include users who should receive emails
        AND (
            -- If user has no preferences, default to sending email
            np.id IS NULL
            OR (
                -- If user is not unsubscribed
                (np.is_unsubscribed = false OR np.is_unsubscribed IS NULL)
                AND (
                    -- Check specific preference based on notification content type
                    CASE p_notification_content_type
                        ...
                        ELSE true  -- Default to true for other types
                    END
                )
            )
        );
